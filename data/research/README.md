...
2026-09-08 등록: ETHFI 외 ENA, MON, STX, CAKE, BNB, ONDO, INJ, XPL. 신규 8종목은 공식 자료 기반 개요·가치 포착 확인 항목·날짜가 확인된 주요 발표를 수동 등록했다. 기사 날짜는 확인일과 다르며, 특히 Plasma의 등록 발표는 2025년 자료다. 최신 뉴스를 빠짐없이 수집했다는 의미는 아니다.

시장 데이터는 각 CoinGecko 식별자에 연결했다. MON 차트는 Bybit 현물, 나머지 신규 종목은 Binance 현물이다. ENA·CAKE만 기존 DefiLlama 프로토콜 수집기를 사용하며, 레이어1의 체인 지표를 프로토콜 지표로 대체하지 않는다. INJ의 초기 배분 언락 완료는 공식 INJ 페이지 기준이며 신규 발행 중단을 뜻하지 않는다. 나머지 신규 종목은 다음 언락 일자·수량 미등록 상태로, 공급 압력 0으로 평가하지 않는다.
