...
function renderSummary(){
  const countedAssets=includedAssets(),values=countedAssets.map(a=>valueKrw(a)),total=values.reduce((a,b)=>a+b,0);$('totalValue').textContent=won.format(total);$('totalMeta').textContent=`USD/KRW ${num.format(fx)} · ${new Date().toLocaleTimeString('ko-KR',{hour:'2-digit',minute:'2-digit'})} 기준${priceErrors.size?` · ${priceErrors.size}개 가격 조회 실패`:''}`;
  headerWidgets?.setUpdatedAt(new Date());
  const allGroups=Object.keys(CLASSES).map(key=>({key,value:countedAssets.filter(a=>a.asset_class===key).reduce((s,a)=>s+valueKrw(a),0)}));
  ...
  const stableTotal=countedAssets.reduce((sum,asset)=>sum+(Number(asset.stable_amount_krw)||0),0);
